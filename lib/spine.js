...
export function coerceJourneyPlan(raw, opts = {}) {
  ...
  const needsRoster = opts.reviewModel === "marketplace" || opts.reviewModel === "hybrid";
  const anchors = needsRoster
    ? [...ANCHORS.slice(0, ANCHORS.findIndex((a) => a.id === BODY_BEFORE)), ROSTER_ANCHOR, ...ANCHORS.slice(ANCHORS.findIndex((a) => a.id === BODY_BEFORE))]
    : ANCHORS;
  ...
  const takePhase = (p, anchor) => {
    const label = clean(p?.label, 60) || anchor?.label || "Phase";
    ...
    if (!steps.length && anchor) {
      (ANCHOR_FALLBACK_STEPS[anchor.id] || []).forEach((t, i) => {
    ...
    return {
      id,
      label,
      week: clean(p?.week, 24) || anchor?.week || "TBC",
