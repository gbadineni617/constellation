...
export const HEALTH_TARGETS = [
  { k: "TM attached to projects",   target: 80 },
  { k: "Projects with glossaries",  target: 50 },
  { k: "Workflow template used",    target: 60 },
  { k: "Reviewer agents used",      target: 30 },
  { k: "Projects reach 100%",       target: 90 },
  { k: "Admins/PMs active (30d)",   target: 80 },
];
...
export const INHERITED_KEYS = new Set(["assets", "provision", "smartwords", "sso", "templates", "aiprofile", "a1", "a2", "a3", "i1", "i2"]);
...
export function resolveStatus(rec, phaseId, key, idx, count, custom) {
  ...
  if (rec.inheritedFrom && INHERITED_KEYS.has(key)) return "done";
  const pi = PHASE_ORDER.indexOf(phaseId);
  const si = PHASE_ORDER.indexOf(rec.stage || "prep");
...
function buildFromPlan(rec) {
  ...
  return raw.map((phase) => {
    ...
    const all = (phase.steps || []).concat(extra);

    const items = all.map((st) => ({
      k: st.k,
      t: st.t,
      custom: st.custom || false,
      s: overrides[st.k] || (st.custom ? "open" : st.status || "open"),
      note: st.note || "",
      due: dues[st.k] || null,
...
export function buildJourney(rec) {
  // A generated journey carries its own phase plan; a template journey is assembled
  // from the three axes. Everything downstream — assess, progressOf, the star map —
  ...
  return raw.filter(Boolean).map((phase) => {
    const dues = rec.dueDates || {};
    const owners = rec.owners || {};
    const extra = ((rec.customItems || {})[phase.id] || []).map((x) => ({ ...x, custom: true }));
    const all = phase.items.concat(extra);
    const items = all.map((it, i) => ({
      ...it,
      s: resolveStatus(rec, phase.id, it.k, i, phase.items.length, it.custom || phase.custom),
      note: N[it.k]
        || (rec.inheritedFrom && INHERITED_KEYS.has(it.k) ? "Inherited from " + rec.inheritedFrom.customer + "." : "")
