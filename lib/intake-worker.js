...
export async function runIntake({ files, onProgress = () => {} }) {
  ...
  try {
    ...
    if (classified.ok) facts = classified.value;
  } catch (e) {
    console.error("[intake] classification failed:", e.message);
  }
...

  {
    ...
    if (last?.type === "text") {
      last.text = last.text + known + formatPatterns(patterns) + formatReferences(references);
    }
  }
  ...
  const intake = coerceIntake({ ...parsed, ...facts });
